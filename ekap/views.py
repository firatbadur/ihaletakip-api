...
_PRO_PARAMS = frozenset({
    ...
    # ⚠️ **`islemlerKuralSeti` kaynaklı DÖRT bayrak bilerek YOK** — o alan bir ihale
    # özelliği değil, giriş yapmış EKAP kullanıcısına özel arayüz durumudur
    # (bkz. `sync.apply_pro_fields`). Üçüncü taraf olarak çağırdığımız için:
    #   • `itirazen_sikayet_var` / `idareye_sikayet_var` / `sikayet_dilekce_var`
    #     → üretimde 1.043.450 ihalenin **sıfırında** true (hep `false`)
    #   • `duzeltme_ilani_var` → üretimde ihalelerin **%82'sinde** true; kaynak anahtar
    #     (`ilanDuzeltmeIlani`) büyük olasılıkla "düzeltme ilanı yayımlanabilir" izni,
    #     "yayımlandı" değil. Her beş ihaleden dördüne düzeltme ilanı çıkmaz.
    # İkisi de aynı hatayı üretir: **kullanıcı filtreyi işaretler, filtrelediğini sanır
    # ama filtrelememiştir.** `=true` ya hiç sonuç ya neredeyse her şey döner — limit
    # kılığına girmiş sessiz bir doğruluk hatası.
    "fiyat_disi_unsur_var", "e_eksiltme_yapilacak",
...
def apply_tender_filters(qs, params):
    ...
    for param in (
        # ⚠️ `duzeltme_ilani_var` BİLEREK YOK — kolon dolduruluyor ama filtre olarak
        # sunulmuyor (%82 true, seçicilik yok). Bkz. `_PRO_PARAMS` yorumu.
        "fiyat_disi_unsur_var", "e_eksiltme_yapilacak",
        "kismi_ihale", "ilansiz_mi",
    ):
