...
ihalelerini `GET /ekap/tenders/?idare_detsis=<detsis_no>` ile listeler. Kayıtlı ihale
(`SavedTender`) desenini izler — **Free limiti = 5** (`FREE_FAVORITE_AUTHORITY_LIMIT`;
Pro'da sınırsız). Mevcut favorinin upsert'i limiti tetiklemez.
...
- **Model**: doğal anahtar `(user, detsis_no)` unique. `detsis_no` ASCII → `/` içermez,
  ...
  mobil yalnızca `detsis_no` gönderir. Serializer'da bu alanlar read-only.
- **Uçlar** (`/api/v1/...`, hepsi JWT):
  - `GET favorite-authorities/` — favori idareleri listele.
  - `POST favorite-authorities/` — gövde `{"detsis_no": "24308110"}`; aynı `detsis_no`
    tekrar → upsert (hata yok).
...
- **Kapılama altyapısı** (`accounts/premium.py`): `PremiumRequired` (APIException, 403,
  ...
  limit sabitleri (`FREE_SAVED_FILTER_LIMIT`/`FREE_SAVED_TENDER_LIMIT`/
  `FREE_FAVORITE_AUTHORITY_LIMIT` = **5**) ve uca özel Türkçe mesajlar. Model importu
  yapmaz → sirküler import yok. Yeni bir premium kısıt eklerken **bu modülü kullan**,
  elle `{success:false}` kurma.
- **Free kısıtları** (Pro'da hepsi sınırsız):
  - Kayıtlı **filtre** > 5 → 403 (`SavedFilterListCreateView.perform_create`).
  - Kayıtlı **ihale** > 5 → 403 (`SavedTenderListCreateView.perform_create`; upsert ile
    **mevcut** İKN güncellemesi limiti tetiklemez, yalnızca yeni kayıt sayılır).
  - Favori **idare** > 5 → 403 (`FavoriteAuthorityListCreateView.perform_create`; upsert ile
    **mevcut** `detsis_no` güncellemesi limiti tetiklemez, yalnızca yeni favori sayılır).
  - **İhale alarmı kurma** (`TenderAlarmListCreateView.perform_create`) → 403; `require_premium`
    ...
    kullanıcının alarmını atlar (Pro iken kurup Free'ye düşen kullanıcıya push gitmez;
    `is_premium` property → Python'da elenir).
  ...
  - **İhale Asistanı bildirimleri** → `match_recommendations` Free üyeyi **atlar**
    ...
    Not: kayıtlı-filtre (`check_saved_filter_matches`) bildirimi asistan bildirimi DEĞİL →
    kısıtlanmadı (alarm bildirimi artık Pro'ya özel — yukarıda).
