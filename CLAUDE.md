...
(`SavedTender`) desenini izler — **Free limiti = 10** (`FREE_FAVORITE_AUTHORITY_LIMIT`;
...
- **Kapılama altyapısı** (`accounts/premium.py`): `PremiumRequired` (APIException, 403,
  ...
  `FREE_FAVORITE_AUTHORITY_LIMIT` = **10**) ve uca özel Türkçe mesajlar. Model importu
...
- **Free kısıtları** (Pro'da hepsi sınırsız):
  - Kayıtlı **filtre** > 10 → 403 (`SavedFilterListCreateView.perform_create`).
  - Kayıtlı **ihale** > 10 → 403 (`SavedTenderListCreateView.perform_create`; upsert ile
    **mevcut** İKN güncellemesi limiti tetiklemez, yalnızca yeni kayıt sayılır).
  - Favori **idare** > 10 → 403 (`FavoriteAuthorityListCreateView.perform_create`; upsert ile
